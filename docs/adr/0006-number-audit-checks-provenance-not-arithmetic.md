...
- **Dates are values, not numbers.** Reading `2025-09-30` as 2025, 9, and 30 would put three small integers into evidence and hand an answer a free "30 months of runway" — the money-shaped claim the audit exists to catch. So a date is matched whole, in whichever of the recognised forms it was written, and never accounts for a bare figure. The cost is at the edges: a date written in a form this does not recognise reads as bare numbers and is reported.
- **A quarter label is a name, not a figure.** `Q3` is what the Datasets call a period, so neither side reads it as the number 3 — otherwise an agent doing as its prompt told it and naming the period would fail the audit for a figure nobody claimed. The audit does not check the label a figure is filed under, and never did: a real number attributed to the wrong quarter passes either way.
